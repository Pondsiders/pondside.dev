...
/**
 * Generate nested slug path from the content entry ID.
 *
 * Astro's glob loader already collapses `folder/index.md` to the folder name,
 * so the ID is the authoritative slug source for both flat and folder-based
 * posts. Hidden segments (prefixed with `_`) are stripped.
 *
 * Examples:
 * "travel/japan/tokyo"   -> "travel/japan/tokyo"   (nested flat post)
 * "lil-transformy"       -> "lil-transformy"       (folder post, index.md)
 * "_2026/my-post"        -> "my-post"              (hidden folder)
 */
